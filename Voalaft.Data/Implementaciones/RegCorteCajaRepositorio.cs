...
namespace Voalaft.Data.Implementaciones
{
    public class RegCorteCajaRepositorio : IRegCorteCajaRepositorio
    {
        ...
        public async Task<List<ImpresionData>> TicketMovimientoCaja(long nIDRegistroCaja)
        {
            List<ImpresionData> impresion = [];
            try
            {
                using (var con = _conexion.ObtenerSqlConexion())
                {
                    ...
                    {
                        Connection = con,
                        CommandText = "RST_TicketMovimientoCaja",
                        CommandType = CommandType.StoredProcedure,
                    };
                    cmd.Parameters.AddWithValue("@nIDRegistroCaja", nIDRegistroCaja);
                    using (var reader = await cmd.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
