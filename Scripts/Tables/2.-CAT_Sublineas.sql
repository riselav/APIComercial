...
/* Create Tables */
IF NOT EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[CAT_Sublineas]') AND OBJECTPROPERTY(id, N'IsUserTable') = 1) 
BEGIN
CREATE TABLE [CAT_Sublineas]
(
	[nSublinea] int NOT NULL,    -- id
	[nLinea] int NOT NULL,    -- id de línea agrupadora
	[cDescripcion] varchar(200) NOT NULL,    -- nombre de la sublínea
	...
	ALTER TABLE [CAT_Sublineas] ADD CONSTRAINT [PK_CAT_Sublineas]
	PRIMARY KEY CLUSTERED ([nSublinea] ASC)
END
GO

/* Create Foreign Key Constraints */
IF not EXISTS (SELECT 1 FROM SysObjects (NOLOCK) where name ='FK_CAT_Sublineas_CAT_Lineas' AND xtype = 'F')
	ALTER TABLE [CAT_Sublineas] ADD CONSTRAINT [FK_CAT_Sublineas_CAT_Lineas]
