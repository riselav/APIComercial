...
/*  Created On : 05-feb.-2025 11:32:19 p. m. 				*/
/*  DBMS       : SQL Server 2012 						*/
/* ---------------------------------------------------- */

/* Create Tables */
IF NOT EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[CAT_Precios]') AND OBJECTPROPERTY(id, N'IsUserTable') = 1) 
BEGIN
CREATE TABLE [CAT_Precios]
(
	[nFolio] int NOT NULL,    -- id
	[nIDArticulo] int NOT NULL,    -- id de catálogo de artículos
	...
	ALTER TABLE [CAT_Precios] ADD CONSTRAINT [PK_CAT_Precios]
	PRIMARY KEY CLUSTERED ([nFolio] ASC)
END
GO

/* Create Foreign Key Constraints */
IF not EXISTS (SELECT 1 FROM SysObjects (NOLOCK) where name ='FK_CAT_Precios_CAT_Articulos' AND xtype = 'F')
	ALTER TABLE [CAT_Precios] ADD CONSTRAINT [FK_CAT_Precios_CAT_Articulos]
	FOREIGN KEY ([nIDArticulo]) REFERENCES [CAT_Articulos] ([nIDArticulo]) ON DELETE No Action ON UPDATE No Action
GO

IF not EXISTS (SELECT 1 FROM SysObjects (NOLOCK) where name ='FK_CAT_Precios_CAT_ListasPrecio' AND xtype = 'F')
	ALTER TABLE [CAT_Precios] ADD CONSTRAINT [FK_CAT_Precios_CAT_ListasPrecio]
	FOREIGN KEY ([nIdLista]) REFERENCES [CAT_ListasPrecio] ([nIdLista]) ON DELETE No Action ON UPDATE No Action
GO

IF not EXISTS (SELECT 1 FROM SysObjects (NOLOCK) where name ='FK_CAT_Precios_CAT_Monedas' AND xtype = 'F')
	ALTER TABLE [CAT_Precios] ADD CONSTRAINT [FK_CAT_Precios_CAT_Monedas]
	FOREIGN KEY ([nIdMoneda]) REFERENCES [CAT_Monedas] ([nMoneda]) ON DELETE No Action ON UPDATE No Action
GO

IF not EXISTS (SELECT 1 FROM SysObjects (NOLOCK) where name ='FK_CAT_Precios_CAT_Sucursales' AND xtype = 'F')
	ALTER TABLE [CAT_Precios] ADD CONSTRAINT [FK_CAT_Precios_CAT_Sucursales]
