...
/* Create Tables */
IF NOT EXISTS (SELECT 1 FROM dbo.sysobjects WHERE id = object_id(N'[CAT_Monedas]') AND OBJECTPROPERTY(id, N'IsUserTable') = 1) 
BEGIN
CREATE TABLE [CAT_Monedas]
(
	[nMoneda] int NOT NULL,    -- id
	[cDescripcion] varchar(200) NOT NULL,    -- nombre del registro
	[cSigno] varchar(3) NULL,    -- signo de la moneda
	...
	ALTER TABLE [CAT_Monedas] ADD CONSTRAINT [PK_CAT_Monedas]
	PRIMARY KEY CLUSTERED ([nMoneda] ASC)
END
GO
If dbo.fn_ExisteCampo('CAT_Monedas','bMonedaBase')=0
	ALTER TABLE CAT_Monedas ADD bMonedaBase bit
GO