...
if dbo.fn_ExisteCampo ('CAT_Clientes', 'nIDRFC')=0
Begin
	Alter table CAT_Clientes ADD nIDRFC bigint
End 

IF not EXISTS (SELECT 1 FROM SysObjects (NOLOCK) where name ='FK_CAT_Clientes_CAT_RFC' AND xtype = 'F')
	ALTER TABLE CAT_Clientes ADD CONSTRAINT FK_CAT_Clientes_CAT_RFC
	FOREIGN KEY (nIDRFC) REFERENCES [CAT_RFC] (nIDRFC) ON DELETE No Action ON UPDATE No Action
GO