...
namespace Voalaft.API.Servicios.Implementacion
{
    public class RegMovimientoVentaServicio:IRegistroVentaServicio
    {
        ...
        public async Task<List<ReporteVentas>> CM_CON_reporte_ventas_sp(ParametrosReporteVentas parametrosReporteVentas)
        {
            try
            {
                return await _registroVentaRepositorio.CM_CON_reporte_ventas_sp(parametrosReporteVentas);
            }
            catch (DataAccessException)
            {
                throw;
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (línea {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo consultar ventas")
                {
                    Metodo = "ParametrosReporteVentas",
                    ErrorMessage = ex.Message,
                };
            }
        }
