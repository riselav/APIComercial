...
namespace Voalaft.API.Servicios.Implementacion
{
    public class CatSucursalesServicio: ICatSucursalesServicio
    {
        ...
        public async Task<CatSucursales> IME_ConfiguracionTicketSucursal(CatSucursales catSucursal)
        {
            try
            {
                return await _SucursalesRepositorio.IME_ConfiguracionTicketSucursal(catSucursal);
            }
            catch (DataAccessException ex)
            {
                throw;
        ...
        public async Task<CatConfiguracionTicketSucursal> ObtenerConfiguracionTicketSucursal(int nSucursal)
        {
            try
            {
                return await _SucursalesRepositorio.ObtenerConfiguracionTicketSucursal(nSucursal);
            }
            catch (DataAccessException ex)
            {
                throw;
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (CatSucursalesServicio {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo obtener la configuración de ticket de la Sucursal")
                {
