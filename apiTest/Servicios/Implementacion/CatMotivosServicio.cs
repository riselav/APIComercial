...
{
    public class CatMotivosServicio : ICatMotivosServicio
    {
        private readonly ICatMotivosRepositorio _catalogosRepositorio;
        private readonly ILogger<CatMotivosServicio> _logger;

        public CatMotivosServicio(ILogger<CatMotivosServicio> logger, ICatMotivosRepositorio catalogosRepositorio)
        {
            _logger = logger;
        ...
        public async Task<List<CatMotivos>> ObtenerPorTipoMotivo(int nTipoMotivo)
        {
            try
            {
                return await _catalogosRepositorio.ObtenerPorTipoMotivo(nTipoMotivo);
            }
            catch (DataAccessException ex)
            {
                throw;
