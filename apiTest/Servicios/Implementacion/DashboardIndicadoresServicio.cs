...
namespace Voalaft.API.Servicios.Implementacion
{
    public class DashboardIndicadoresServicio : IDashboardIndicadoresServicio
    {
        ...
        public async Task<TopSellingDetail> ObtenerDetallePlatillosMasVendidos(int n_Sucursal, int n_FechaInicial, int n_FechaFinal)
        {
            try
            {
                return await _dashboardIndicadoresRepositorio.ObtenerDetallePlatillosMasVendidos(n_Sucursal, n_FechaInicial, n_FechaFinal);
            }
            catch (DataAccessException ex)
            {
                string className = ex.StackTrace != null ? ex.StackTrace.Split('\n')[0].Trim().Split(' ')[0] : "";
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (línea {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo obtener detalle de platillos más vendidos")
                {
                    Metodo = "ObtenerDetallePlatillosMasVendidos",
                    ErrorMessage = ex.Message,
                };
            }
        }
        ...
        public async Task<TopProfitableDetail> ObtenerDetallePlatillosMasRedituables(int n_Sucursal, int n_FechaInicial, int n_FechaFinal)
        {
            try
            {
                return await _dashboardIndicadoresRepositorio.ObtenerDetallePlatillosMasRedituables(n_Sucursal, n_FechaInicial, n_FechaFinal);
            }
            catch (DataAccessException ex)
            {
                string className = ex.StackTrace != null ? ex.StackTrace.Split('\n')[0].Trim().Split(' ')[0] : "";
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (línea {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo obtener detalle de platillos más redituables")
                {
                    Metodo = "ObtenerDetallePlatillosMasRedituables",
                    ErrorMessage = ex.Message,
                };
            }
        }
