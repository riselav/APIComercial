...
namespace Voalaft.API.Servicios.Implementacion
{
    public class CatClientesServicio: ICatClientesServicio
    {
        ...
        public async Task<ContactoCliente> EliminarContactoCliente(ContactoCliente contacto)
        {
            try
            {
                return await _catClientesRepositorio.EliminarContactoCliente(contacto);
            }
            catch (DataAccessException ex)
            {
                throw;
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (línea {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo eliminar el contacto del cliente")
                {
        ...
        public async Task<CatCorreoContactoRFC> EliminarCorreoCliente(CatCorreoContactoRFC correo)
        {
            try
            {
                return await _catClientesRepositorio.EliminarCorreoCliente(correo);
            }
            catch (DataAccessException ex)
            {
                throw;
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (línea {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo eliminar el correo de contacto del cliente")
                {
        ...
        public async Task<List<CatClientes>> ConsultaClientesVenta(ParametrosConsultaClientes paramClientes)
        {
            try
            {
                return await _catClientesRepositorio.ConsultaClientesVenta(paramClientes);
            }
            catch (DataAccessException ex)
            {
                throw;
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError($"Error en {className}.{methodName} (línea {lineNumber}): {ex.Message}");
                throw new ServiciosException("Error(srv) No se pudo obtener información del cliente")
                {
